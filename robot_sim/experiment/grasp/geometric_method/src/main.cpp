    ...
    while (ros::ok()) //ros::ok()返回false会停止运行，进程终止。
    {
        if (RGB_update_flag && Depth_update_flag)
        {
            ...
            if (enable_move_and_grasp_flag)
            {
                ...
                grasp(0.33);
                go_home();
                move(-0.55, 0.24, 1.4, 0.0, 0.0, 0.0); //箱子上边
                grasp(0.8);
                go_home();
            }
            ...
            if (enable_move_tcp_eelink_flag)
            {
                enable_move_tcp_eelink_flag = false;
                move(tcp_eelink_x, tcp_eelink_y, tcp_eelink_z, tcp_eelink_roll, tcp_eelink_pitch, tcp_eelink_yaw);
                ROS_INFO("move tcp_eelink done!\n");
            }
            if (enable_move_tcp_eelink_and_grasp_flag)
            {
                enable_move_tcp_eelink_and_grasp_flag = false;
                move(tcp_eelink_x, tcp_eelink_y, tcp_eelink_z + 0.2, tcp_eelink_roll, tcp_eelink_pitch, tcp_eelink_yaw);
                move(tcp_eelink_x, tcp_eelink_y, tcp_eelink_z + 0.1, tcp_eelink_roll, tcp_eelink_pitch, tcp_eelink_yaw);
                move(tcp_eelink_x, tcp_eelink_y, tcp_eelink_z + 0.0, tcp_eelink_roll, tcp_eelink_pitch, tcp_eelink_yaw);
                grasp(0.33);
                go_home();
                move(-0.55, 0.24, 1.4, 0.0, 0.0, 0.0); //箱子上边
                grasp(0.8);
            ...
            if (enable_move_forward_flag)
            {
                enable_move_forward_flag = false;
                move_forward(shoulder_pan_joint, shoulder_lift_joint, elbow_joint, wrist_1_joint, wrist_2_joint, wrist_3_joint);
            }
        }
