...
class MoveItIkDemo:
	...
	#末端位置2，右手预备位置1   
        target_pose2.pose.position.x = -0.25
	...
	#末端位置2，右手预备位置1   
        target_pose4.pose.position.x = 0.22
        target_pose4.pose.position.y = 0.88
        target_pose4.pose.position.z = 1.5
        #末端姿态，四元数
        ...
        # traj = ur.plan()
        plan_success, traj4, planning_time, error_code = ur.plan()
	...
	#末端位置2，右手预备位置1   
        target_pose5.pose.position.x = 0.22
        target_pose5.pose.position.y = 0.88
        target_pose5.pose.position.z = 1.1
        #末端姿态，四元数
        target_pose5.pose.orientation.x = 0
        target_pose5.pose.orientation.y = 0
        target_pose5.pose.orientation.z = 0
        target_pose5.pose.orientation.w = 1
        ...
        ur.set_pose_target(target_pose5, end_effector_link)

        # 规划运动路径，返回虚影的效果

        # traj = ur.plan()
        plan_success, traj5, planning_time, error_code = ur.plan()
	...
	#末端位置2，右手预备位置1   
        target_pose6.pose.position.x = 0.22
        target_pose6.pose.position.y = 0.88
        target_pose6.pose.position.z = 1.5
        #末端姿态，四元数
        target_pose6.pose.orientation.x = 0
        target_pose6.pose.orientation.y = 0
        target_pose6.pose.orientation.z = 0
        target_pose6.pose.orientation.w = 1
        ...
        ur.set_pose_target(target_pose6, end_effector_link)

        # 规划运动路径，返回虚影的效果

        # traj = ur.plan()
        plan_success, traj6, planning_time, error_code = ur.plan()
